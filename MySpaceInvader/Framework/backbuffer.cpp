...
void 
BackBuffer::DrawAnimatedSprite(AnimatedSprite& animatedSprite,int currentX)
{
	SDL_Rect sourceRect;
	sourceRect.x = currentX;
	sourceRect.y = 0;
	sourceRect.w = animatedSprite.getFrameWidth();
	sourceRect.h = animatedSprite.GetHeight();

	SDL_Rect destRect;
	destRect.x = animatedSprite.GetX();
	destRect.y = animatedSprite.GetY();
	destRect.w = 64;
	destRect.h = 64;

	SDL_RenderCopy(m_pRenderer, animatedSprite.GetTexture()->GetTexture(), &sourceRect, &destRect);
}
...
BackBuffer::CreateAnimatedSprite(const char* pcFilename)
{
	...
	pSprite->SetFrameSpeed(0.15f);
	pSprite->SetFrameWidth(64);
